...
if [ $CMP_RESULT -eq 1 ] && [[ "$LEVEL" == "0" ]]; then
    ...
    logger "$0: T_ON=$T_ON, T_OFF=$T_OFF, T=$T Set relay to ON";
fi
...
if [ $CMP_RESULT -eq 1 ] && [[ "$LEVEL" == "1" ]]; then
    ...
    logger "$0: T_ON=$T_ON, T_OFF=$T_OFF, T=$T Set relay to OFF";
fi

OLD_LEVEL=$LEVEL
LEVEL=`gpio -g read 11`;
logger "$0: T_ON=$T_ON, T_OFF=$T_OFF, T=$T, OLD_LEVEL=$OLD_LEVEL, LEVEL=$LEVEL";
